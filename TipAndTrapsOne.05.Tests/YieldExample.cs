...
namespace TipAndTrapsOne._05.Tests
{
    ...
    public class YieldExample
    {
        ...
        IEnumerable<int> GetTwoAnd6(IEnumerable<int> numbers)
        {
            ...
            {
                Trace.WriteLine("GetTwoAnd6: Processing input number: " + number);

                if (number == 2 || number == 6)
                {
                    Trace.WriteLine("GetTwoAnd6: yielding: " + number);
        ...
        IList<int> GetEvenNumbers(IEnumerable<int> numbers)
        {
            ...
            foreach (var number in numbers)
            {
                Trace.WriteLine("GetEvenNumbers: Processing input number: " + number);
        ...
        IEnumerable<int> GetEvenNumbersUsingYield(IEnumerable<int> numbers)
        {
            foreach (var number in numbers)
            {
                Trace.WriteLine("GetEvenNumbersUsingYield: Processing input number: " + number);

                if (number % 2 == 0)
                {
                    Trace.WriteLine("GetEvenNumbersUsingYield: yielding: " + number);
                    yield return number;
                }
            }
        }
